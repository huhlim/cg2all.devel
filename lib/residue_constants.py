...
class Residue(object):
    ...
    def add_torsion_info(self, tor_s):
        for tor in tor_s:
            ...
            if tor.name == 'CHI':
                ...
                self.torsion_xi_periodic[index, periodic] = 1.
            else:
    ...
    def add_rigid_group_info(self, rigid_group, transform):
        ...
        for info in transform:
            self.transform.append(info[:-1] + \
                [(np.array(info[-1][0]), np.array(info[-1][1]))])
...
def get_rigid_group_by_torsion(residue_name, tor_name, index=-1, sub_index=-1):
    ...
    rigid_group[1] = np.array(rigid_group[1]) / 10. # in nm
    if len(rigid_group[0]) == 0:
        raise ValueError(f"Cannot find rigid group for {residue_name} {tor_name} {index} {sub_index}\n")
...
def get_rigid_transform_by_torsion(residue_name, tor_name, index, sub_index=-1):
    ...
    for X, Y, tR in rigid_group_transformations[residue_name]:
        if (X[0] == tor_name and X[1] == index) and (sub_index < 0 or X[2] == sub_index):
            rigid_transform = (np.array(tR[1]), np.array(tR[0])/10.)
            break
