...
def get_new_residue_numbers(pdb):
    atom_index_N = pdb.top.select("name N")
    atom_index_C = pdb.top.select("name C")
    #
    seg_no = 0
    chain_index = -1
    resNo_s = [{}, {}]
    segNo_s = {}
    for (iN, iC) in zip(atom_index_N, atom_index_C):
        residue_N = pdb.top.atom(iN).residue
        residue_C = pdb.top.atom(iC).residue
        assert residue_N.index == residue_C.index
        residue = residue_N
        #
        ...
        if xyz_prev is not None:
            d = np.linalg.norm(xyz_N - xyz_prev)
            if d > CHAIN_BREAKs:
                i_res += 1
                seg_no += 1
        #
